...
const boldAllLinks = (text) => {
    return text.replace(/https?:\/\/[^\s]+/g, (url) => $.underline().bold(url));
};
const log = (message, type = "info") => {
    let logMessage = boldAllLinks(message);
    switch (type) {
        case "tip":
            logMessage = $.blue().bold("tip: ") + $.white(logMessage);
            break;
        case "warn":
            logMessage = $.yellow().bold("WARNING: ") + $.yellow(logMessage);
            break;
        case "error":
            logMessage = $.red().bold("ERROR: ") + $.red(logMessage);
            break;
        case "success":
            logMessage = $.green().bold("SUCCESS: ") + $.green(logMessage);
            break;
    }
    console.log(logMessage);
};

const RPC_PROVIDER_BASE_DOMAIN = "satsignal.io";
const RPC_PROVIDER = "https://runes." + RPC_PROVIDER_BASE_DOMAIN + "/v1";
const NODE_REPO = "https://github.com/runeapeslabs/Nana";
...
const asap = typeof queueMicrotask !== 'undefined' ?
  queueMicrotask.bind(_global) : ( typeof process$1 !== 'undefined' && nextTick || _setImmediate);
...
class Runes3 {
    constructor(rpc_endpoint) {
        this.rpc_endpoint = "";
        if (!rpc_endpoint) {
            log("No RPC endpoint was provided, default RPC from satsignal will be used instead.", "warn");
            log(`You can get your own endpoint for free at https://${RPC_PROVIDER_BASE_DOMAIN} to get rid of this warning or run your own node with: ${NODE_REPO}`, "tip");
        }
        //Use rpc_endpoint provided or public endppoint from satsignal
        this.rpc_endpoint = rpc_endpoint ?? RPC_PROVIDER;
    }
    // Add a method to the class
    getAccount(address, private_key) {
        if (!private_key)
            return new AnonymousAccount({ address, __Runes3: this });
